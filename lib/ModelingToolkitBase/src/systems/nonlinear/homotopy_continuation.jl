...
function display_reason(reason::NonPolynomialReason.T, sym)
    ...
    elseif reason == NonPolynomialReason.UnrecognizedOperation
        ...
        error("This should never happen. Please open an issue in ModelingToolkitBase.jl.")
    end
end
...
function handle_rational_polynomials(x, wrt; fraction_cancel_fn = simplify_fractions)
    ...
    elseif op == *
        ...
        error("Unhandled operation in `handle_rational_polynomials`. This should never happen. Please open an issue in ModelingToolkitBase.jl with an MWE.")
    end
