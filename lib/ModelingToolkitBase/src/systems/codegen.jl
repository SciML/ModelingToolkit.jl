...
"""
    build_explicit_observed_function(sys, ts; kwargs...) -> f or (f_oop, f_iip)

Generate a callable that evaluates symbolic output expressions `ts` from a system state,
inputs, parameters, and independent variable. The requested expressions must not have cyclic
observed dependencies.

# Arguments

- `sys::AbstractSystem`: The system that provides the symbolic context.
- `ts`: A symbolic variable or expression, or a collection of symbolic expressions, to
  evaluate.

# Keywords

- `inputs = nothing`: Additional symbolic variables supplied as an argument after the state.
- `disturbance_inputs = nothing`: Unknown disturbance inputs removed from the parameter
  arguments and not added as function arguments.
- `known_disturbance_inputs = nothing`: Disturbance inputs removed from the parameter
  arguments and supplied as a final function argument.
- `ps = parameters(sys; initial_parameters = true)`: Parameters available to the generated
  function.
- `return_inplace = Val(false)`: For vector-valued `ts`, return both out-of-place and
  in-place callables. `Val(true)` and `Val(false)` are preferred over booleans.
- `output_type = Array`: Container type for an out-of-place vector-valued result.
- `param_only::Bool = false`: Omit the state argument and only permit parameter-dependent
  expressions.
- `expression::Bool = false`: Return generated Julia expressions instead of callables.
- `eval_expression::Bool = false`: Evaluate generated code in `eval_module`.
- `eval_module::Module = @__MODULE__`: Module used when `eval_expression = true`.
- `checkbounds::Bool = false`: Check bounds while destructuring parameters. The default
  generated code uses `@inbounds`.
- `throw::Bool = true`: Throw when `ts` references symbols absent from `sys`.
- `wrap_delays = is_dde(sys) && !param_only`: Include a history-function argument for DDEs.
- `force_time_independent::Bool = false`: Omit the independent-variable argument.
- `compiler_options::CompilerOptions = CompilerOptions()`: Compiler configuration for
  evaluated generated code.
...
For a scalar `ts`, returns one out-of-place callable. For a collection and
`return_inplace = Val(true)`, returns `(f_oop, f_iip)`. The standard signatures are
`f_oop(unknowns, inputs, p..., t, known_disturbance_inputs)` and
`f_iip(output, unknowns, inputs, p..., t, known_disturbance_inputs)`, omitting arguments
that are not applicable. The parameter arguments are splatted because `MTKParameters` can
produce more than one parameter argument.
...
(; io_sys) = generate_control_function(sys, [u]; simplify = true)
