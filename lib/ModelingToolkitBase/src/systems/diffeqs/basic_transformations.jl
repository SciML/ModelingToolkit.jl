...
```julia
using ModelingToolkitBase, OrdinaryDiffEq, Test
...
θ0 = 0.1
g(x) = x[1]^2
demod = ModelingToolkitBase.Girsanov_transform(de, u; θ0=0.1)
...
function respecialize(sys::AbstractSystem, mapping; all = false)
    ...
    for element in ps_to_specialize
        ...
        if iscall(k)
            op = operation(k)::SymbolicT
            @assert !iscall(op)
            op = SU.Sym{VartypeT}(nameof(op); type = SU.FnType{Tuple, T, Nothing}, shape = SU.shape(k))
            args = arguments(k)
            new_p = op(args...)
        else
