...
@testset "Basics" begin
    ...
    function check_contract(sys)
        state = ModelingToolkitBase.get_tearing_state(sys)
        ...
        for (i, eq) in enumerate(eqs)
            ...
            actual = filter(!ModelingToolkitBase.isparameter, collect(actual))
...
@testset "Compose/extend" begin
    ...
    @unpack foo = goo
    @test value(ModelingToolkitBase.initial_conditions(goo)[foo.a]) == 3
    @test value(ModelingToolkitBase.initial_conditions(goo)[foo.b]) == 300
end
...
@named foo = Circuit()
@test mtkcompile(foo) isa ModelingToolkitBase.AbstractSystem
