...
@test hasbounds(u)
@test ModelingToolkitBase.dump_variable_metadata(u).bounds == (-1, 1)

@variables y
@test !hasbounds(y)
@test !haskey(ModelingToolkitBase.dump_variable_metadata(y), :bounds)
...
@test ModelingToolkitBase.dump_variable_metadata(y).guess == 0

# Default
@variables y = 0
@test ModelingToolkitBase.getdefault(y) == 0
@test ModelingToolkitBase.hasdefault(y) == true
@test ModelingToolkitBase.dump_variable_metadata(y).default == 0

# Issue#2653
@variables y[1:3] [guess = ones(3)]
@test getguess(y) == ones(3)
@test hasguess(y) == true
@test ModelingToolkitBase.dump_variable_metadata(y).guess == ones(3)

for i in 1:3
    @test getguess(y[i]) == 1.0
    @test hasguess(y[i]) == true
    @test ModelingToolkitBase.dump_variable_metadata(y[i]).guess == 1.0
end

@variables y
@test hasguess(y) == false
@test !haskey(ModelingToolkitBase.dump_variable_metadata(y), :guess)

# Disturbance
@variables u [disturbance = true]
@test isdisturbance(u)
@test ModelingToolkitBase.dump_variable_metadata(u).disturbance

@variables y
@test !isdisturbance(y)
@test !haskey(ModelingToolkitBase.dump_variable_metadata(y), :disturbance)

# Tunable
@parameters u [tunable = true]
@test istunable(u)
@test ModelingToolkitBase.dump_variable_metadata(u).tunable

@parameters u2 [tunable = false]
@test !istunable(u2)
@test !ModelingToolkitBase.dump_variable_metadata(u2).tunable

@parameters y
@test istunable(y)
@test ModelingToolkitBase.dump_variable_metadata(y).tunable
...
@test hasdist(u)
@test getdist(u) == d
@test ModelingToolkitBase.dump_variable_metadata(u).dist == d

@parameters y
@test !hasdist(y)
@test !haskey(ModelingToolkitBase.dump_variable_metadata(y), :dist)
...
param_meta = ModelingToolkitBase.dump_parameters(sys)
...
@test ModelingToolkitBase.dump_variable_metadata(u).desc == "This is my input"

@variables u
@test getdescription(u) == ""
@test !hasdescription(u)
@test !haskey(ModelingToolkitBase.dump_variable_metadata(u), :desc)
...
params_meta = ModelingToolkitBase.dump_parameters(sys)
params_meta = Dict([ModelingToolkitBase.getname(meta.var) => meta for meta in params_meta])
...
@test ModelingToolkitBase.dump_variable_metadata(x).connect == Flow
x = ModelingToolkitBase.setconnect(x, ModelingToolkitBase.Stream)
@test getconnect(x) == ModelingToolkitBase.Stream

struct BadConnect end
@test_throws Exception ModelingToolkitBase.setconnect(x, BadConnect)

# Unit
@variables x [unit = u"s"]
@test hasunit(x)
@test getunit(x) == u"s"
@test ModelingToolkitBase.dump_variable_metadata(x).unit == u"s"

# Misc data
@variables x [misc = [:good]]
@test hasmisc(x)
@test getmisc(x) == [:good]
x = ModelingToolkitBase.setmisc(x, "okay")
@test getmisc(x) == "okay"

# Variable Type
@variables x
@test ModelingToolkitBase.getvariabletype(x) == ModelingToolkitBase.VARIABLE
@test ModelingToolkitBase.dump_variable_metadata(x).variable_type == ModelingToolkitBase.VARIABLE
@test ModelingToolkitBase.dump_variable_metadata(x).variable_source == :variables
x = ModelingToolkitBase.toparam(x)
@test ModelingToolkitBase.getvariabletype(x) == ModelingToolkitBase.PARAMETER
@test ModelingToolkitBase.dump_variable_metadata(x).variable_source == :variables

@parameters y
@test ModelingToolkitBase.getvariabletype(y) == ModelingToolkitBase.PARAMETER
