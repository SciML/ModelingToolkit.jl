...
using ModelingToolkitBase: MTKParameters, ParameterIndex, NONNUMERIC_PORTION
...
vars = @variables y(t) = 2.0 dy(t)
pars = @parameters begin
    interpolator::Interpolator = Interpolator(x, dt)
    k = 2.0
end
eqs = [
    D(y) ~ k * dy
    dy ~ get_value(interpolator, t)
]
...
if @isdefined(ModelingToolkit)
    using ModelingToolkitBase, LinearAlgebra
    using ModelingToolkitStandardLibrary.Mechanical.Rotational
    using ModelingToolkitStandardLibrary.Blocks
    using ModelingToolkitStandardLibrary.Blocks: t
    using ModelingToolkitBase: connect
...
@testset "Callable parameters" begin
    @testset "As FunctionWrapper" begin
        ...
        @test value(ModelingToolkitBase.initial_conditions(sys)[fn]) == _f1
