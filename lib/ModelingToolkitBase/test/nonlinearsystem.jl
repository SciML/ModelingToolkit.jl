...
using ModelingToolkitBase: get_default_or_guess, MTKParameters
...
@test ModelingToolkitBase.jacobian_sparsity(ns).colptr == sparse(Num.(jac)).colptr
@test ModelingToolkitBase.jacobian_sparsity(ns).rowval == sparse(Num.(jac)).rowval
...
@testset "Can convert from `System`" begin
    ...
    prob = NonlinearProblem(sys3, [y => 1.0, q => 2.0])
