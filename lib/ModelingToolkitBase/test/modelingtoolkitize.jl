...
limit(a, N) = ModelingToolkitBase.ifelse(a == N + 1, 1, ModelingToolkitBase.ifelse(a == 0, N, a))
...
@test !ModelingToolkitBase.hasbounds(unknowns(sys)[1])
@test !ModelingToolkitBase.hasbounds(unknowns(sys)[2])
@test ModelingToolkitBase.hasbounds(unknowns(sys)[3])
@test ModelingToolkitBase.getbounds(unknowns(sys)[3]) == (1.0, 2.0)
...
@testset "`modelingtoolkitize(::SDEProblem)` sets initial conditions" begin
    ...
    @test length(ModelingToolkitBase.initial_conditions(sys)) == length(u0) + length(p)
