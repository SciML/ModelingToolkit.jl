...
@testset "`respecialize`" begin
    ...
    @test ModelingToolkitBase.iscomplete(sys2)
    @test ModelingToolkitBase.is_split(sys2)
    ps = ModelingToolkitBase.get_ps(sys2)
    idx = findfirst(isequal(rp), ps)
    @test value(initial_conditions(sys2)[rp]) == Bar()
    @test symtype(ps[idx]) <: Bar
    ic = ModelingToolkitBase.get_index_cache(sys2)
