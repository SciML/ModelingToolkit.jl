...
rxs = [Reaction(k[1], nothing, [A]),            # 0 -> A
       ...
       Reaction(k[12], [A,B,C], [C,D], [1,3,4], [Poisson(2), 3]),          # A+3B+4C -> k_pois * C + 3D
...
function sdenoise(u,k,t)
       ...
       G = [sqrt(k[1]) z  z z;
            ...
            # burst variances
            z z sqrt(k[12]*A*B^3*C^4/144) * sqrt(2.) z;
...
u  = rand(length(k))
t  = 0.
G  = vcat(fill(p[21],8),fill(p[22],4),fill(p[23],8),[p[22],p[23],p[23]])' .* sdenoise(u,p,t)
@variables η[1:3]
sdesys_noise_scaling = convert(SDESystem,rs;noise_scaling=vcat(fill(η[1],8),fill(η[2],4),fill(η[3],8)))
sf = SDEFunction{false}(sdesys_noise_scaling, states(rs), parameters(sdesys_noise_scaling))
G2 = sf.g(u,p,t)
@test norm(G-G2) < 100*eps()
...
# jumps[17] = ConstantRateJump((u,p,t) -> p[17]*u[1]*exp(u[2])*binomial(u[3],2), integrator -> (integrator.u[3] -= 2; integrator.u[4] += rand(Bernoulli(1))))
