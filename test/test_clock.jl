...
using ModelingToolkit: Inferred, merge_inferred, merge_domains, get_time_domain,
                       collect_operator_variables, get_eq_domain, vars
...
function Base.getindex(sol::ODESolution, var::Num, t::AbstractArray, from::Symbol = :right)
    ...
    map(t) do t
        ...
        ind === nothing &&
            throw(ArgumentError("t = $t is not a valid time point in the solution."))
        u[ind]
    end
end
...
@testset "preprocess_hybrid_equations" begin
    ...
    @test isequal(ModelingToolkit.normalize_shifts(eq, Dict()),
                  Shift(t, 1)(u) ~ -Shift(t, -1)(u))
    ...
    expected_eqs = [Difference(t; dt = 1, update = true)(ud) ~ Shift(t, 0)(ud) +
                                                               ud_delay[1] - ud_delay[2];
    ...
    new_eqs = part.eqs
    ud_delay = part.vars[(length(dvs) + 1):end]
    expected_eqs = [Difference(t; dt = 1, update = true)(ud) ~ ud_delay[1] - ud_delay[2];
                    Difference(t; dt = 1, update = true)(ud_delay[1]) ~ ud;
                    Difference(t; dt = 1, update = true)(ud_delay[2]) ~ ud_delay[1]]
    @test all(isequal.(new_eqs, expected_eqs))
    ...
    new_eqs = part.eqs
    ud_delay = part.vars[(length(dvs) + 1):end]
    expected_eqs = [Difference(t; dt = 1, update = true)(ud) ~ -ud_delay[2];
                    Difference(t; dt = 1, update = true)(ud_delay[1]) ~ ud;
                    Difference(t; dt = 1, update = true)(ud_delay[2]) ~ ud_delay[1]]
    @test all(isequal.(new_eqs, expected_eqs))
    ...
    eqs = [Shift(t, 3)(ud) ~ Shift(t, 2)(ud) + Shift(t, 1)(ud) - Shift(t, 0)(ud)
...
connections = [f.u ~ Sample(d)(-1)#(t >= 1)  # step input
