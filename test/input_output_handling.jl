...
@testset "generate_control_function with disturbance inputs" begin
    for split in [true, false]
        ...
        @named sys = System(eqs, t)
        f, dvs, ps, io_sys = ModelingToolkit.generate_control_function(
            sys; inputs = [u], disturbance_inputs = [d], simplify, split)
        ...
        @named sys = System(eqs, t)
        f, dvs, ps, io_sys = ModelingToolkit.generate_control_function(
            sys; inputs = [u], disturbance_inputs = [d],
...
@testset "With callable symbolic" begin
    ...
    eqs = [D(x) ~ -x + p(u)]
    @named sys = System(eqs, t)
    f, dvs, ps, io_sys = ModelingToolkit.generate_control_function(sys; inputs = [u])
