...
let
    ...
    sys_bot_ss = mtkbuild(sys_bot)
    sys_mid2_ss = mtkbuild(sys_mid2)
    sys_mid1_ss = mtkbuild(sys_mid1)
    sys_top_ss = mtkbuild(sys_top)
    ...
    # Checks `unknowns`. O(t) is eliminated by `mtkbuild` and
