@fallback_iip_specialize function JumpProcesses.JumpProblem{iip, spec}(
    ...
    if (has_vrjs || has_eqs)
        if has_eqs && has_noise
            prob = SDEProblem{iip, spec}(
                ...
                build_initializeprob = false, checkbounds, cse, kwargs...)
        elseif has_eqs
            prob = ODEProblem{iip, spec}(
                sys, u0map, tspan, pmap; check_compatibility = false,
                build_initializeprob = false, checkbounds, cse, kwargs...)
        else
            _, u0, p = process_SciMLProblem(EmptySciMLFunction, sys, u0map, pmap;
        ...
        _f, u0, p = process_SciMLProblem(EmptySciMLFunction, sys, u0map, pmap;
    ...
    cbs = process_events(sys; callback, eval_expression, eval_module,
