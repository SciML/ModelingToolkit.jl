...
@fallback_iip_specialize function SemilinearODEFunction{iip, specialize}(
        ...
        stiff_linear = true, stiff_quadratic = false, stiff_C = false,
    ...
    f1, f2 = generate_semiquadratic_functions(
        sys, A, B, C; stiff_linear, stiff_quadratic, stiff_C, expression, wrap_gfw = Val{true},
    ...
    if jac
        ...
        _W_sparsity = get_semiquadratic_W_sparsity(
            sys, A, B, C, Cjac; stiff_linear, stiff_quadratic, stiff_C, mm = M)
        W_prototype = calculate_W_prototype(_W_sparsity; u0, sparse)
    else
        _jac = nothing
        W_prototype = nothing
...
@fallback_iip_specialize function SemilinearODEProblem{iip, spec}(
        ...
        stiff_linear = true, stiff_quadratic = false, stiff_C = false, jac = false, kwargs...) where {
        iip, spec}
    ...
    f, u0, p = process_SciMLProblem(SemilinearODEFunction{iip, spec}, sys, op;
        ...
        semiquadratic_form, sparse, u0_eltype, stiff_linear, stiff_quadratic, stiff_C, jac, kwargs...)
