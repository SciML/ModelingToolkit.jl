...
function problem_ctors(prob::Type{<:SemilinearODEProblem}, istd)
    ...
    """
        SciMLBase.$prob(sys::System, op, tspan::NTuple{2}; kwargs...)
        SciMLBase.$prob{iip}(sys::System, op, tspan::NTuple{2}; kwargs...)
        SciMLBase.$prob{iip, specialize}(sys::System, op, tspan::NTuple{2}; stiff_linear = true, stiff_quadratic = false, stiff_C = false, kwargs...)
    """
end
...
part of the equations is the stiff part, but the keywords `stiff_linear`, `stiff_quadratic` and `stiff_C` can
...
Note that all three of `stiff_linear`, `stiff_quadratic`, `stiff_C` cannot be identical, and at least
