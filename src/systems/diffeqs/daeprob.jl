...
function DAEFunction(sys::AbstractODESystem, args...; kwargs...)
    nums = eq_to_num.(sys.eqs)
    f = eval(build_function(nums, D.(sys.states), sys.states, sys.ps, sys.iv))
    DAEFunction(f, args...; kwargs...)
end
