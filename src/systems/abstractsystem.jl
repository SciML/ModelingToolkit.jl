...
Base.write(io::IO, sys::T) where {T <: AbstractODESystem} = write(io, string(toexpr(sys)))
Base.read(io::IO, sys::T) where {T <: AbstractODESystem} = eval(Meta.parse(read(io, String)))
