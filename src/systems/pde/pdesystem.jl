...
struct PDESystem <: ModelingToolkit.AbstractMultivariateSystem
	...
	@add_kwonly function PDESystem(eqs, bcs, domain, ivs, dvs,
		...
		eqs = eqs isa AbstractVector ? eqs : [eqs]
		bcs = bcs isa AbstractVector ? bcs : [bcs]
		ivs = ivs isa AbstractVector ? ivs : [ivs]
		dvs = dvs isa AbstractVector ? dvs : [dvs]
		ps = (ps isa AbstractVector) | (ps isa SciMLBase.NullParameters) ? ps : [ps]
		...
		domain = Symbolics.scalarize(domain)
