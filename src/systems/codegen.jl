...
function generate_semiquadratic_functions(sys::System, A, B, C; stiff_linear = true,
        stiff_quadratic = false, stiff_C = false, expression = Val{true}, wrap_gfw = Val{false},
        eval_expression = false, eval_module = @__MODULE__, kwargs...)
    if A === nothing && B === nothing
        throw(ArgumentError("Cannot generate split form for the system - it has no linear or quadratic part."))
    end

    if (stiff_linear || A === nothing) && (stiff_quadratic || B === nothing) && stiff_C
    ...
    if B !== nothing
        ...
        B_vals = map(eachindex(eqs)) do i
            ...
            if stiff_quadratic == stiff_C && C !== nothing
    ...
    if A !== nothing
        ...
        retain_old = stiff_linear == stiff_quadratic && B !== nothing ||
                     stiff_linear == stiff_C && C !== nothing
        push!(A_ir,
            Assignment(:__tmp_A,
