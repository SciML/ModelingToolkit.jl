...
"""
function generate_semiquadratic_functions(sys::System, A, B, C; stiff_A = true,
        stiff_B = false, stiff_C = false, expression = Val{true}, wrap_gfw = Val{false},
        eval_expression = false, eval_module = @__MODULE__, kwargs...)
    if A === nothing && B === nothing
        throw(ArgumentError("Cannot generate split form for the system - it has no linear or quadratic part."))
    end

    if (stiff_A || A === nothing) && (stiff_B || B === nothing) && stiff_C
    ...
    # A
    if A !== nothing
        A_ir = stiff_A ? f1_iip_ir : f2_iip_ir
        retain_old = stiff_A == stiff_B && B !== nothing ||
                     stiff_A == stiff_C && C !== nothing
        push!(A_ir,
            Assignment(:__tmp_A,
    ...
    f2_terms = []
    if A !== nothing
        push!(stiff_A ? f1_terms : f2_terms, term(*, linear_matrix_param, oop_x))
    end
    if B !== nothing
        ...
        push!(stiff_B ? f1_terms : f2_terms, MakeArray(B_elems, oop_x))
    end
    if C !== nothing
        push!(stiff_C ? f1_terms : f2_terms, MakeArray(C, oop_x))
    end
...
function generate_semiquadratic_jacobian(
        sys::System, A, B, C, Cjac; sparse = false, stiff_A = true, stiff_B = false,
        stiff_C = false, expression = Val{true}, wrap_gfw = Val{false},
        eval_expression = false, eval_module = @__MODULE__, kwargs...)
    if sparse
...
function get_semiquadratic_W_sparsity(
        sys::System, A, B, C, Cjac; stiff_A = true, stiff_B = false,
        stiff_C = false, mm = calculate_massmatrix(sys))
    eqs = equations(sys)
    dvs = unknowns(sys)
    M = length(eqs)
    N = length(dvs)
    jac = spzeros(Num, M, N)
    if stiff_A && A !== nothing
        tmp = wrap.(A)
        jac .+= tmp
    end
    if stiff_B && B !== nothing
        for (i, mat) in enumerate(B)
            mat === nothing && continue
            jac[i, :] .+= (mat + transpose(mat)) * dvs
        end
    end
    if stiff_C && C !== nothing
        jac .+= Cjac
    end
